...
class LocationFlutterPlugin {
  ...
  // 关闭前台定位服务 （仅支持Android）
  Future<bool> disableLocInForeground() async {
    if (Platform.isIOS) {
      print('iOS暂不支持disableLocInForeground');
      return false;
    } else {
      return BMFLocationDispatcherFactory.instance.optionsDispatcher
          .setDisableLocInForeground(_channel);
    }
  }

  // 开发者应用如果有后台定位需求（仅支持Android）
  // 在退到后台的时候，为了保证定位可以在后台一直运行， 可以调用该函数；
  // 会将定位SDK的SERVICE设置成为前台服务，适配ANDROID 8后台无法定位问题，其他版本下也会提高定位进程存活率
  // notifationId: 为通知栏notifation设置唯一id，必须大于0
  // title: 开发者自定义通知的标题
  // text: 开发者自定义通知的文本
  Future<bool> enableLocInForeground(
      String notifationId,String notifationName, String title, String text) async {
    if (Platform.isIOS) {
      print('iOS暂不支持enableLocInForeground');
      return false;
    } else {
      return BMFLocationDispatcherFactory.instance.optionsDispatcher
          .setEnableLocInForeground(_channel, notifationId,notifationName, title, text);
    }
  }
